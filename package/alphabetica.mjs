...
function buildClass(name, spec) {
  ...
  // Computed-property class-name pattern: no `new Function`, no runtime
  // string compilation. CSP-safe, minifier-transparent, source-map-clean.
  const Ctor = ({
...
function unifyOne(a, b, sub) {
  ...
  // Bare wildcard (_) matches anything, no binding.
  if (typeof aw === "function" && aw[WILDCARD] === true) return sub;
  if (typeof bw === "function" && bw[WILDCARD] === true) return sub;
